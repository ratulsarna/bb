...
it("publishes only its own maintenance providers without a display and only measures the requested resource", async () => {
  ...
  try {
    ...
    expect(await read("online", "codex")).toMatchObject({
      accountKey: null,
      usage: {
        status: "ok",
        plan: null,
    ...
    await read("online", "codex", true);
    expect(collect).toHaveBeenCalledTimes(2);
    expect(await read("offline", "codex")).toMatchObject({
      observedAt: null,
      usage: { status: "error" },
    });
    expect(collect).toHaveBeenCalledTimes(2);
    ...
    await expect(read("online", "codex")).rejects.toThrow("no longer exists");
  } finally {
...
it("forwards validated provider-owned identity and normalization metadata while tolerating older providers", async () => {
  const { bb, harness } = createFakePluginHost({
    sdk: {
      ...
      providers: {
        list: async () => [
          { id: "codex", displayName: "Custom", pluginId: "provider-codex" },
        ],
      },
      system: {
        usageLimits: async () => ({
  ...
  try {
    ...
    const value = usageMeasurementSchema.parse(
      await harness.behavior.callRpc(usageFetchMethod, {
        resourceId: JSON.stringify(["host", "codex"]),
        refresh: false,
      }),
    );
