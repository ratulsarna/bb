...
describe("bb settings commands", () => {
  ...
  it("updates the Cloud AI experiment while preserving other flags", async () => {
    const put = vi.fn(async ({ json }) => json);
    stubServerApi({
      "v1.system.config.$get": vi.fn(async () => ({
        generalSettings: defaultAppSettings,
    ...
    await runCommand(["settings", "experiment", "cloudAi", "true"], register);

    expect(put).toHaveBeenCalledWith({
      json: { ...defaultExperiments, cloudAi: true },
    });
  });
