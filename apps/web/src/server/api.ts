...
export interface Deps {
  ...
  baseDomain: string;
  appUrl: string;
  /** Authoritative gate origin template; `{label}` is the routing label. */
...
export function resolveServerUrlTemplate(
  ...
  if (!configured) return `https://{label}.${baseDomain}`;
  if (configured.split("{label}").length !== 2) {
    throw new Error(
      "CONNECT_SERVER_URL_TEMPLATE must contain {label} exactly once",
    );
  }
  const probe = "bb-label-probe";
  const url = new URL(configured.replace("{label}", probe));
  if (
    ...
    throw new Error(
      "CONNECT_SERVER_URL_TEMPLATE must be an HTTP(S) origin in the form {label}.BASE_DOMAIN",
    );
  }
...
export interface AccountState {
  ...
  appUrl: string;
  baseDomain: string;
  /** Same template used for previews and the authoritative pairing response. */
  serverUrlTemplate: string;
...
export interface IssuedCode {
  code: string;
  expiresInMs: number;
  /** Account/redeem origin; distinct from serverUrl in local development. */
  baseUrl: string;
  serverUrl: string;
  serverId: string;
}
 ...
 * this field to build share URLs — it must not be the account's primary handle
 * when a non-primary server was paired. `serverUrl` is the authoritative gate
 * origin; production and local development both derive it from the label via
 * the boundary-validated server URL template.
 *
...
> {
  ...
  const srv = await db
    ...
    .where(eq(server.id, row.serverId))
    .get();
  if (!srv) return { error: "invalid-code", status: 404 };
  // Routing label of the redeemed server (not necessarily the account handle).
  const handle = srv.subdomain;
  const serverUrl = serverUrlForLabel(handle, serverUrlTemplate);
  return {
    credential,
    ...
    tunnelUrl: `${serverUrl.replace(/^http/u, "ws")}/__tunnel`,
  };
}
