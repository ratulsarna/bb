...
| Todo and context indicators       | Inspect thread show --json pendingTodos and the context ring during a multi-step synthetic turn; compare `thread context <id> --json` with the card. On Claude Code, expand details after a completed turn; verify categories, animated disclosure, and compact layout. Clear context and verify the old breakdown disappears. | Counts, plan/todo states, and supported usage data agree with timeline events.                                                                                  |
