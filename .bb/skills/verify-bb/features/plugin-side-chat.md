...
| Source deletion  | Directly delete a disposable source with a used side chat, queued input, a running fork, nested hidden forks, and a visible fork. Exercise stop failure/reconnect and fork creation racing deletion in the isolated server harness. | Explicit lifecycle dependents are deleted recursively; ownership links persist until storage cleanup succeeds. Queued work stays blocked and cleanup retries on reconnect. Visible forks and ordinary children retain their existing behavior. |
...
- Verify sourceThreadId and checkpoint linkage on the hidden fork. Side chats have parentThreadId=null and lifecycleOwnerThreadId=sourceThreadId; lifecycle ownership is separate from sidebar and notification semantics. Source: `plugins/side-chat/server.ts:99`.
