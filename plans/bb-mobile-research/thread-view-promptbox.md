...
- Permission requests / ask-user-question: `PendingInteraction` list (`useThreadPendingInteractions`, WS kind `interactions-changed`), rendered above composer by `ThreadPendingInteractionBanner` (`components/thread/pending-interactions/ThreadPendingInteractionBanner.tsx:73-104`): approval subjects `command|file_change|permission_grant|plan`, decisions `allow_once|allow_for_session|deny` (`packages/domain/src/pending-interactions.ts:115-119`), resolved via `sdk.threads.interactions.resolve`; native and plugin user questions share `packages/shared-ui/src/components/ui/question-form.tsx` and the pure answer-state helpers in `packages/shared-ui/src/components/ui/question-form-state.ts`. The native `UserQuestionAnswerForm` adapter (`user-questions/UserQuestionInteractionContent.tsx`) posts `{kind:"user_answer",answers}`; the plugin adapter (`plugins/ask-user-question/app.tsx`) submits `{answers}`. App-specific integration lives in `PendingInteractionShell.tsx` (disclosure and presentation) and `ThreadQuestionFormHost.tsx` (focused-pane command bindings). Historical state also appears inline as `approval`/`question` work rows (`QuestionWorkRowBody.tsx`). Child-thread attention: `useChildThreadPendingAttention` (`child-thread-pending-interactions.ts:53`). Composer blocked while pending (`buildFollowUpSubmitMode`, `views/thread-detail/threadDetailPromptSubmission.ts:126-146`).
- Steer/queue/stop: `FollowUpSubmitMode` ready|queue|blocked (`packages/client-core/src/prompt/follow-up-submit-mode.ts`); Enter → `mode:"queue-if-active"`, Cmd+Enter → `"steer-if-active"` (`threadDetailPromptSubmission.ts:201-231`), `steerActiveThreadOnEnter` setting flips it (`ThreadDetailView.tsx:2679`); pending steers are tail conversation rows (`turnRequest.kind==="steer"`, label `conversation-turn-request-label.ts`); stop → `useStopThread` + optimistic "Stop requested" system row (`ThreadTimelineSurface.tsx:93-140`). Queue shown by `QueuedMessagesList` (dnd-kit reorder).
...
- packages/shared-ui/src/components/ui/question-form-state.ts: **reusable-as-is** — Pure answer-state initialization, validation, answer construction, and shortcut-choice helpers shared by native and plugin question adapters.
- packages/shared-ui/src/components/ui/question-form.tsx: **headless-logic-only** — Shared web question UI and state transitions; JSX uses DOM controls, Tailwind, textarea sizing, and browser focus behavior, so React Native needs its own rendering layer.
- components/thread/pending-interactions/ThreadPendingInteractionBanner.tsx, PendingInteractionShell.tsx, and user-questions/ThreadQuestionFormHost.tsx: **headless-logic-only** — App-specific submission, disclosure, presentation, and focused-pane command integration. Decision/label helpers and @bb/core-ui formatters are pure; the shell uses NavLink and Tailwind, and the shortcut host depends on app command and pane contexts.
